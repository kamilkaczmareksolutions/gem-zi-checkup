...
Problem w tym, ze czysta teoria nie uwzglednia **realnych kosztow**: przewalutowania (XTB inkasuje 0.5% na kazdej konwersji PLN/USD), brak akcji ulamkowych (BOSSA), podatek Belki, slippage. Ten projekt odpowiada na konkretne pytania:

1. **XTB IKE czy BOSSA IKE?** -- co jest lepsze dla strategii rotacyjnej przy malym kapitale (~9k PLN)?
...
|---------|-----------|-------|
| Broker | **BOSSA IKE** | +12,000 PLN vs XTB na 14 lat (58k vs 46k z 9k startu) |
| Deadband | **4.0%** | IS optimum 5.2%, OOS walk-forward srednia 2.2% -- kompromis |
| Wiecej ETF-ow | **Nie** | U5 (Sharpe 1.13) >> U7 (0.79) >> U9 (0.75) |
| Rachunek opodatkowany | **Katastrofa** | -26,000 PLN vs IKE (podatek Belki niszczy strategia rotacyjna) |
...
```
Co miesiac:
  1. Oblicz momentum = Price[t-1] / Price[t-12] - 1    (skip ostatni miesiac)
...
│   ├── momentum.py          # Momentum 12-1 + dual momentum (selekcja celu)
│   ├── broker.py            # Modele kosztowe: XTB, BOSSA, rachunek opodatkowany
...
| 2 | Baseline GEM (5 ETF, deadband=0) na 4 brokerach | Krzywe kapitalowe, trade logi |
...
| 7 | Scenariusze z wplatami, crossover XTB/BOSSA | `decision_memo.md` |
...
| Prowizja | 0% | 0% | 0.29% (min 14 PLN) | 0% |
| Slippage | 0.10% | 0.10% | 0.10% | 0.10% |
| Akcje ulamkowe | TAK | NIE | NIE | TAK |
| Podatek od zysku | 0% (IKE) | 0% (IKE) | 0% (IKE) | 19% |
| **Koszt rotacji** | **~1.2%** | **~0.2%** | **~0.78%+** | **~1.2% + podatek** |

## Testy odpornosci

- **Walk-forward**: 36 mies. trening / 12 mies. test / 11 foldow. Sredni OOS return: 11.85%/rok.
- **Timing luck**: CAGR od 12.4% do 19.0% w zaleznosci od dnia rebalancingu (rozstrzal 6.6 pp).
- **Czulosc FX**: kazde 0.25 pp kosztu FX zjada ~5,000-6,000 PLN wartosci koncowej.
